...
OBS向け表示は背景、パネル、NIC情報を表示せず、透明背景へ重ねます。LatencyとNIC Trafficは独立したBrowser Sourceとして配置します。ルート `/` は稼働確認と利用案内のページであり、OBS表示には使用しません。
...
各表示URLで `parts` を利用できます。たとえばNIC通信量のグラフだけを表示する場合は `http://127.0.0.1:8080/traffic?parts=graph` を使用します。未指定または有効な値がない場合は、空画面を避けるため数値とグラフの両方を表示します。
...
ネットワーク計測と数値更新は約1秒間隔ですが、グラフはサンプル時刻を基準にCanvasへ最大60fpsで描画し、60秒の時間窓を滑らかに移動します。数値は補間や更新エフェクトを加えず、受信した実測値へ即時更新します。グラフの曲線は左側目盛りの直後にある固定境界でクリップし、目盛り側へは描画しません。最新の履歴項目が有効な間は、その実測値を現在時刻まで水平に保持して固定された右端まで表示します。
...
| 測定値が更新されない | ルートのヘルスチェックを確認し、OBSのURLが `/latency` または `/traffic` か確認する |
| OBS向け表示が見切れる | Latencyは480 x 270、NIC Trafficは480 x 210にする |
...
- 統計は直近60サンプル、表示は直近60秒を対象とし、描画余白を含む65秒以上の履歴をメモリ上に保持する（安全上限256件、永続化しない）
- 設定の動的再読み込みは非対応
